...
if (argc != 3):
	sys.stderr.write('Usage: # python %s tagname dir_path\n' % argvs[0])
	quit()
...
saving_direcory_path = argvs[2] + "_" + tagname
# 画像データの保存ファイルパス ( @Notice 書き出しは厳密なJSONフォーマットではないので読むときは良いようにする必要がある)
datafile = os.path.join(saving_direcory_path, "data.json")
...
os.makedirs(saving_direcory_path, exist_ok=True)
...
for page in range(page_start, page_start + page_num):
	...
	for illust in json_result.response:
		...
		imagefile = os.path.join(saving_direcory_path, imagefilename)
		...
		aapi.download(illust.image_urls.large, path=saving_direcory_path)
		sleep(sleep_sec)
		#aapi.download(illust.image_urls.large, path=saving_direcory_path, name=imagefilename)
